...
> 删除指定 File（元数据与内容，产出文件连带清理 `session_outputs` 映射行）。引用检查不通过时返回 400：被未归档会话挂载、或是未归档会话的产出，二者都拒绝删除；已归档会话的挂载与产出不阻止。不存在或无权限时返回 404。
...
| 状态码 | error.type | 场景 |
| --- | --- | --- |
| 400 | `invalid_request_error` | 被未归档会话挂载，或属于未归档会话的产出（已归档会话不阻止删除） |
