...
> 分页列出指定 Agent 的历史版本。每条记录都是该版本的完整 Agent 配置快照，而不是仅包含版本号的摘要。

## 请求示例

```bash
curl -sS "http://127.0.0.1:8787/v1/agents/$AGENT_ID/versions?limit=20&order=desc" \
...
- 分页参数与 [list-agent.md](list-agent.md) 相同（见 [README.md](README.md#分页)），默认按 `version` 倒序。
- 每个条目的时间戳是**版本级**的（各版本有自己的 `created_at` / `updated_at`），`archived_at` 保持 **Agent 级**（同一 Agent 的所有版本回显同一值）。
...
paths:
  /v1/agents/{agentId}/versions:
    get:
      tags: [Agent]
      summary: 列出 Agent 版本
      description: 分页列出指定 Agent 的历史版本。每条记录都是该版本的完整 Agent 配置快照，而不是仅包含版本号的摘要。
    ...
    Agent:
      ...
      properties:
        ...
        archived_at:
          type: string
          nullable: true
          description: 归档时间；未归档时为 null。Agent 级字段，所有版本回显同一值。
      required:
        - id
        - type
        - name
