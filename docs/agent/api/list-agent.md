...
> 分页列出当前身份可读取的 Agent（含已归档）。支持通过 `limit`、`order` 和 `page` 游标翻页。

## 请求示例

```bash
curl -sS "http://127.0.0.1:8787/v1/agents?limit=20&order=desc" \
...
分页说明见 [README.md](README.md#分页)。nano 实现按 `(created_at, id)` keyset 排序（默认 `desc`，即最新创建的在前）；游标对客户端 opaque。
...
paths:
  /v1/agents:
    get:
      tags: [Agent]
      summary: 列出 Agent
      description: 分页列出当前身份可读取的 Agent（含已归档）。支持通过 limit、order 和 page 游标翻页。
