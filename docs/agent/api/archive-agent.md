...
> 归档指定 Agent。该操作幂等；归档是 Agent 的终止操作——归档后 Agent 变为只读：新会话不能再引用它，已有会话继续运行，版本历史仍可读取。

## 请求示例

```bash
curl -sS -X POST "http://127.0.0.1:8787/v1/agents/$AGENT_ID/archive" \
  -H "Authorization: Bearer $NANO_API_KEY"
```

响应 `200`（`archived_at` 已填充；重复归档返回相同结果）：
...
```json
{
  ...
  "archived_at": "2026-09-10T02:30:00.000Z"
}
```

说明：

- 与 GLM 一致，Agent 没有 delete 端点；归档即终止，`archived_at` 一旦填充不可清除。
- 归档后的 Agent 拒绝更新（见 [update-agent.md](update-agent.md)，返回 400）。
- GLM 中归档会同时归档仍在运行的 Deployment；nano 一期没有 Deployment 资源，此联动留待引入 Deployment 时补充。
...
paths:
  /v1/agents/{agentId}/archive:
    post:
      tags: [Agent]
      summary: 归档 Agent
      description: 归档指定 Agent。该操作幂等，归档是 Agent 的终止操作；归档后 Agent 只读，新会话不可再引用。
