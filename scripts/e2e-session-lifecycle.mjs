...
try {
  ...
  const session = (await client.createSession({
    ...
    title: "e2e lifecycle",
    initial_events: [{ type: "user.message", content: [{ type: "text", text: "bootstrap hello" }] }],
  })).body;
  ...
  mock.enqueue({
    ...
    tool_calls: [{ name: "bash", arguments: JSON.stringify({ command: "echo lifecycle-ok" }) }],
    usage: { prompt_tokens: 5, completion_tokens: 2 },
  });
  await client.sendEvents(session.id, [
    { type: "user.message", content: [{ type: "text", text: "please lifecycle-tool now" }] },
  ]);
  ...
  assert(resumed.filter((e) => e.type === "session.status_idle").at(-1).stop_reason.type === "end_turn", "续跑 end_turn");
  console.log("✓ 审批 allow → 执行 → 续跑完成");

  // 5. interrupt:慢流中掐断
  mock.enqueue({ match: "lifecycle-slow", chunks: [{ content: "wor", delayMs: 9000 }, { content: "d" }] });
  await client.sendEvents(session.id, [
    { type: "user.message", content: [{ type: "text", text: "please lifecycle-slow now" }] },
  ]);
  ...
  assert(!interrupted.slice(interrupted.findIndex((e) => e.type === "user.interrupt")).some((e) => e.type === "agent.message"), "无半截终事件");
