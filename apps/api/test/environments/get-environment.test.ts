...
import { applyMigrations, createDefaultEnvironment, getEnvironment, jsonBody, archiveEnvironmentInDb, type EnvironmentJson, type ErrorEnvelope } from "./helpers";

beforeAll(applyMigrations);

describe("GET /v1/environments/{environmentId}", () => {
  it("创建后获取,字段与创建响应完全一致", async () => {
    const created = await createDefaultEnvironment({ name: "fetch-me", description: "d" });
