...
import { applyMigrations, createDefaultEnvironment, jsonBody, listEnvironments, type EnvironmentJson, type ErrorEnvelope, type PageJson } from "./helpers";
...
describe("GET /v1/environments 分页", () => {
  it("默认参数返回 20 条、按创建时间倒序", async () => {
    ...
    for (let i = 0; i < 25; i++) {
      created.push(await createDefaultEnvironment({ name: `page-env-${String(i).padStart(2, "0")}` }));
    }
    ...
    while (cursor !== null) {
      const next = await jsonBody<PageJson<EnvironmentJson>>(await listEnvironments(`?page=${encodeURIComponent(cursor)}`));
      all.push(...next.data);
  ...
  it("order=asc 正序", async () => {
    const desc = await jsonBody<PageJson<EnvironmentJson>>(await listEnvironments("?limit=100&order=desc"));
    const asc = await jsonBody<PageJson<EnvironmentJson>>(await listEnvironments("?limit=100&order=asc"));
