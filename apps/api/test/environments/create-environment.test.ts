...
describe('POST /v1/environments 成功路径', () => {
  it('文档示例创建成功,响应回显归一化后的完整配置', async () => {
    ...
    expect(body.id).toMatch(
      /^env_[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/,
    );
...
describe('POST /v1/environments 校验失败返回 400', () => {
  ...
  it('config 内未知字段被拒绝', async () => {
    await expectInvalid({
      name: 'a',
      config: { type: 'cloud', image: 'ubuntu:24.04' },
    });
  });
  ...
  it('包名含空白', async () => {
    await expectInvalid(
      {
        name: 'a',
        config: { type: 'cloud', packages: { pip: ['pandas >=2.0'] } },
      },
      'whitespace',
    );
  });
  ...
  it('unrestricted 携带 limited 专属字段', async () => {
    await expectInvalid({
      ...
      config: {
        type: 'cloud',
        networking: { type: 'unrestricted', allowed_hosts: ['a.com'] },
      },
    });
  });
