...
async function threeVersionSkill(): Promise<string> {
  const skill = await createDefaultSkill({ "SKILL.md": skillMd("hist-skill", "v1") });
...
describe("GET /v1/skills/{skillId}/versions", () => {
  it("三个版本默认倒序 3、2、1", async () => {
    ...
    const page = await jsonBody<PageJson<SkillVersionJson>>(
  ...
  it("把 list-skills 的游标传入返回 400(kind 防混用)", async () => {
    ...
    const res = await listSkillVersions(skillId, `?page=${encodeURIComponent(skillsPage.next_page!)}`);
  ...
  it("limit=0 返回 400;不带凭证返回 401;不存在的 Skill 返回 404", async () => {
    ...
    expect((await listSkillVersions("skill_01911111-0000-7000-8000-000000000000")).status).toBe(404);
  });
});
