...
import { getDb, findSkill, insertNextSkillVersionAndAdvance } from "@nano/db";
...
describe("POST /v1/skills/{skillId}/versions 成功路径", () => {
  ...
  it("空壳 Skill(版本删空)可继续上传,版本号从分配器当前值递增", async () => {
    ...
    const db = getDb(env);
    const v1 = await exports.default.fetch(
      `http://example.com/v1/skills/${skill.id}/versions/1`,
      { method: "DELETE", headers: { Authorization: "Bearer dev-key-change-me" } },
    );
    ...
    const next = await jsonBody<SkillVersionJson>(
      await postSkillVersion(skill.id, versionForm()),
    );
...
describe("POST /v1/skills/{skillId}/versions 错误路径", () => {
  ...
  it("不带凭证返回 401", async () => {
    const skill = await createDefaultSkill();
    const res = await postSkillVersion(skill.id, versionForm(), { Authorization: "" });
...
describe("版本号分配的并发安全(CAS)", () => {
  it("repo 层:expectedVersion 过期时返回 false 且不留孤儿行", async () => {
    ...
    const tree = await normalizeSkillTree([{ path: "SKILL.md", bytes: new TextEncoder().encode(skillMd()) }]);
  ...
  it("并行两次上传:版本号互不重复,不出现 500", async () => {
    ...
    const bodies = await Promise.all(
      results.map(async (res) => (res.status === 201 ? await jsonBody<SkillVersionJson>(res) : null)),
    );
