...
describe("POST /v1/agents/{agentId} 更新语义", () => {
  ...
  it("改 model 字符串简写后 effort 按新模型默认值补全", async () => {
    const created = await createDefaultAgent({ name: "switch-model", model: "glm-5.3" });
    const res = await updateAgent(created.id, { version: 1, model: "glm-5.3-flash" });
    expect(res.status).toBe(200);
  ...
  it("system 传 null 清空", async () => {
    const created = await createDefaultAgent({ name: "clear-sys", model: "glm-5.3", system: "to be cleared" });
  ...
  it("tools 传 [] 清空;但 skills 非空时清空 tools 被合并校验拒绝", async () => {
    const plain = await createDefaultAgent({ name: "plain", model: "glm-5.3", tools: [{ type: "agent_toolset_20260601" }] });
    ...
    const rejected = await updateAgent(withSkills.id, { version: 1, tools: [] });
    expect(rejected.status).toBe(400);
    const envelope = await jsonBody<ErrorEnvelope>(rejected);
    expect(JSON.stringify(envelope.error.details?.issues ?? [])).toContain("agent_toolset_20260601");
  });
  ...
  it("metadata 按键合并:null 删键、新键新增、未提及键保留", async () => {
    ...
    const res = await updateAgent(created.id, { version: 1, metadata: { b: null, c: "3" } });
    expect(res.status).toBe(200);
    expect((await jsonBody<AgentJson>(res)).metadata).toEqual({ a: "1", c: "3" });
  });
  ...
  it("tools 与 mcp_servers 整体替换并保持一一对应", async () => {
    ...
    const res = await updateAgent(created.id, {
      version: 1,
      tools: [
        { type: "agent_toolset_20260601", configs: [{ name: "bash", enabled: false }] },
  ...
  it("无变化:提交与当前一致的配置不升版本,version 与时间戳保持,响应体一致", async () => {
    const created = await createDefaultAgent({ name: "nochange", model: "glm-5.3", system: "same" });
...
describe("POST /v1/agents/{agentId} 并发与版本冲突", () => {
  it("携带旧 version 在二次更新后重放返回 409", async () => {
    const created = await createDefaultAgent({ name: "conflict", model: "glm-5.3" });
    const first = await updateAgent(created.id, { version: 1, system: "v2" });
    expect(first.status).toBe(200);

    const replay = await updateAgent(created.id, { version: 1, system: "stale write" });
  ...
  it("直改库推高版本后以旧期望更新,同样返回 409 且数据不被污染", async () => {
    const created = await createDefaultAgent({ name: "raced", model: "glm-5.3" });
    const raced = await bumpAgentVersionInDb(created.id);
    const res = await updateAgent(created.id, { version: 1, system: "too late" });
  ...
  it("省略 version 时覆盖式更新成功", async () => {
    const created = await createDefaultAgent({ name: "overwrite", model: "glm-5.3" });
...
describe("POST /v1/agents/{agentId} 拒绝路径", () => {
  it("已归档的 Agent 更新返回 400", async () => {
    const created = await createDefaultAgent({ name: "archived", model: "glm-5.3" });
  ...
  it("请求含 mcp_toolset 但不带 mcp_servers 返回 400", async () => {
    const created = await createDefaultAgent({ name: "needs-servers", model: "glm-5.3" });
    const res = await updateAgent(created.id, {
      version: 1,
  ...
  it("version 为 0 或非整数返回 400", async () => {
    const created = await createDefaultAgent({ name: "bad-version", model: "glm-5.3" });
  ...
  it("不带凭证返回 401", async () => {
    const created = await createDefaultAgent({ name: "authz", model: "glm-5.3" });
