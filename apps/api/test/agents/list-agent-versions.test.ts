...
async function createThreeVersionAgent(): Promise<AgentJson> {
  const created = await createDefaultAgent({ name: "history", model: "glm-5.3", system: "v1" });
  await sleep(5);
...
describe("GET /v1/agents/{agentId}/versions", () => {
  ...
  it("各版本有版本级时间戳:递增且 created_at === updated_at(不可变快照)", async () => {
    const agent = await createThreeVersionAgent();
    const page = await jsonBody<PageJson<AgentJson>>(await listAgentVersions(agent.id, "?order=asc"));
  ...
  it("游标翻页:limit=2 取前两页,不重不漏", async () => {
    const agent = await createThreeVersionAgent();
    const page1 = await jsonBody<PageJson<AgentJson>>(await listAgentVersions(agent.id, "?limit=2"));
