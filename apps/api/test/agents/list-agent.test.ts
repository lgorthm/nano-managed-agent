...
describe('GET /v1/agents 默认分页与游标往返', () => {
  it('25 个 Agent:默认 20 条倒序,翻页取完剩余 5 条且 next_page 为 null', async () => {
    ...
    const second = await jsonBody<PageJson<AgentJson>>(
      await listAgents(`?page=${first.next_page}`),
    );
  ...
  it('limit=5 生效且游标连续翻页不重不漏', async () => {
    ...
    const page2 = await jsonBody<PageJson<AgentJson>>(
      await listAgents(`?limit=5&page=${page1.next_page}`),
    );
...
describe('GET /v1/agents 归档与认证', () => {
  it('已归档的 Agent 仍出现在列表中', async () => {
    const archived = await createDefaultAgent({
      name: 'to-archive',
      model: 'glm-5.3',
    });
