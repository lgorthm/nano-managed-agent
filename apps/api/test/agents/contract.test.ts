...
const AGENT_KEYS = [
  "id", "type", "name", "description", "model", "system", "tools",
  "skills", "mcp_servers", "metadata", "multiagent", "version",
...
describe("六个端点的响应契约(docs/agent/api/*.md)", () => {
  ...
  it("GET /v1/agents/{agentId} → 200,响应为完整 ManagedAgent", async () => {
    const created = await createDefaultAgent({ name: "contract-get", model: "glm-5.3" });
  ...
  it("POST /v1/agents/{agentId} → 200,响应为完整 ManagedAgent", async () => {
    const created = await createDefaultAgent({ name: "contract-update", model: "glm-5.3" });
  ...
  it("GET /v1/agents/{agentId}/versions → 200,条目为 ManagedAgent", async () => {
    const created = await createDefaultAgent({ name: "contract-versions", model: "glm-5.3" });
  ...
  it("POST /v1/agents/{agentId}/archive → 200,响应为完整 ManagedAgent", async () => {
    const created = await createDefaultAgent({ name: "contract-archive", model: "glm-5.3" });
...
describe("非 2xx 响应的错误信封契约", () => {
  ...
  it("409:携带过期 version 更新", async () => {
    const created = await createDefaultAgent({ name: "contract-409", model: "glm-5.3" });
