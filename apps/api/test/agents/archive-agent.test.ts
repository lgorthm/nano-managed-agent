...
describe("POST /v1/agents/{agentId}/archive", () => {
  it("归档后 archived_at 填充,重复归档返回完全相同的响应", async () => {
    const created = await createDefaultAgent({ name: "to-archive", model: "glm-5.3" });
    ...
    expect({ ...archived, archived_at: null }).toEqual({ ...created, archived_at: null });
  ...
  it("归档后再次 GET,archived_at 与归档响应一致且不再变化", async () => {
    const created = await createDefaultAgent({ name: "stable-ts", model: "glm-5.3" });
  ...
  it("归档后 GET 与 GET /versions 仍可读", async () => {
    const created = await createDefaultAgent({ name: "readable", model: "glm-5.3" });
  ...
  it("归档后 Agent 仍出现在列表中", async () => {
    const created = await createDefaultAgent({ name: "still-listed", model: "glm-5.3" });
    await archiveAgentViaApi(created.id);
  ...
  it("端到端回归:归档后更新返回 400(M4 用直改库造状态,这里补真实路径)", async () => {
    const created = await createDefaultAgent({ name: "locked", model: "glm-5.3" });
    await archiveAgentViaApi(created.id);
