...
import { ModelHttpError, streamChatCompletion, type ChatModelConfig } from "../../src/runtime/model-client";
...
describe("工具调用分片累积", () => {
  it("续块的 name:null 不覆盖首块工具名,arguments 分片按 index 重组", async () => {
    ...
    expect(result.usage).toEqual({ input_tokens: 11, output_tokens: 7, cache_read_input_tokens: 3 });
    // mock 把每个调用的 arguments 一分为二、续块 name 置 null:重组必须无损
    expect(result.toolCalls).toHaveLength(2);
    expect(result.toolCalls[0]).toMatchObject({ id: "call_mock_0", name: "bash" });
    expect(result.toolCalls[0]!.arguments).toBe(JSON.stringify({ command: "echo hi" }));
    expect(result.toolCalls[1]).toMatchObject({ id: "call_mock_1", name: "ls" });
    expect(result.toolCalls[1]!.arguments).toBe("{}");
  });

  it("上游非 2xx 抛 ModelHttpError(不重试的 4xx)", async () => {
    await enqueueModelScript({ match: "fragment-parse-test", status: 400, chunks: [] });
    await expect(
      streamChatCompletion(CONFIG, [{ role: "user", content: "go" }], { onDelta: () => {} }),
