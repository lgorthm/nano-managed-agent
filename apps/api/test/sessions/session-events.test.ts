...
describe('POST /v1/sessions/{id}/events — 发送事件与模型循环生命周期', () => {
    ...
    expect(
      (
        await sendEvents(session.id, {
          events: Array.from({ length: 11 }, () => TEXT_MESSAGE),
        })
      ).status,
    ).toBe(400);
    expect((await sendEvents(session.id, [{ type: 'user.define_outcome' }])).status).toBe(400);
    expect((await sendEvents(session.id, [{ type: 'user.message', content: [] }])).status).toBe(
      400,
    );
    expect(
      (
  ...
  it('不存在与会话 404;已归档会话拒绝新事件 409', async () => {
    expect(
      (await sendEvents('sess_01911111-9999-7999-8999-999999999999', [TEXT_MESSAGE])).status,
    ).toBe(404);
    const session = await createDefaultSession();
...
describe('M1 模型循环 — 上下文装配与凭据上行', () => {
  it('system 提示词与 model 随 agent_config 快照上行;凭据经 env 注入', async () => {
    ...
    await pollForEvent(session.id, (list) =>
      list.some((event) => event.type === 'session.status_idle'),
    );
...
describe('M1 模型循环 — 打断与恢复', () => {
  it('流中途 user.interrupt:掐断模型请求,不落半截终事件(§4.4)', async () => {
    ...
    const res = await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'please interrupt-me now' }],
      },
    ]);
    expect(res.status).toBe(200);
  ...
  it('强制逐出后恢复:外发 system.message,同上下文重发,终事件无重复(§6 策略 1)', async () => {
    ...
    const res = await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'please recover-me' }],
      },
    ]);
    expect(res.status).toBe(200);
...
describe('GET /v1/sessions/{id}/events/stream — SSE 订阅', () => {
  ...
  it('event_deltas[] 非法值 400;不存在的会话 404;不带凭证 401', async () => {
    const session = await createDefaultSession();
    expect(
      (await openEventStream(session.id, '?event_deltas[]=agent.message&event_deltas[]=bogus'))
        .status,
    ).toBe(400);
...
describe('M4 收尾 — 错误分诊与 stats 投影', () => {
  it('上游 5xx:session.error 带分类消息(HTTP 状态),会话回 idle 可继续', async () => {
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'please fail-500 now' }],
      },
    ]);
    const events = await pollForEvent(session.id, (list) =>
      list.some((event) => event.type === 'session.status_idle'),
    ...
    expect(String(events.find((event) => event.type === 'session.error')!.message)).toContain(
      'HTTP 500',
    );
    expect(events.find((event) => event.type === 'session.status_idle')!.stop_reason).toEqual({
      type: 'interrupted',
...
describe('控制面与运行时的联动', () => {
  it('更新配置实际变更时外发 session.updated;无变化更新不外发', async () => {
    ...
    await pollForEvent(session.id, (list) =>
      list.some((event) => event.type === 'session.updated'),
    );
