...
async function suspendWithAskCall(
  sessionId: string,
  marker: string,
): Promise<{ idle: EventJson; toolUse: EventJson }> {
  ...
  await sendEvents(sessionId, [
    {
      type: 'user.message',
      content: [{ type: 'text', text: `please ${marker}` }],
    },
  ]);
  const events = await pollForEvent(sessionId, (list) =>
    list.some((event) => event.type === 'session.status_idle'),
  );
...
describe('M3 确认挂起与恢复 — always_ask(§4.3)', () => {
  it('挂起:requires_action 带 event_ids,不执行工具,usage 照常落库,会话回 idle', async () => {
    ...
    expect(idle.stop_reason).toEqual({
      type: 'requires_action',
      event_ids: [toolUse.id],
    });
    expect((await eventList(session.id)).some((event) => event.type === 'agent.tool_result')).toBe(
      false,
    );
  ...
  it('部分确认不续跑:全部待审批处理完才回 running;确认按原顺序执行', async () => {
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'please ask-partial' }],
      },
    ]);
    const suspended = await pollForEvent(session.id, (list) =>
      list.some((event) => event.type === 'session.status_idle'),
    );
    ...
    await sendEvents(session.id, [
      { type: 'user.tool_confirmation', tool_use_id: ids[0]!, result: 'allow' },
    ]);
    ...
    await sendEvents(session.id, [
      { type: 'user.tool_confirmation', tool_use_id: ids[1]!, result: 'allow' },
    ]);
    const events = await pollForEvent(
      session.id,
      (list) => list.filter((event) => event.type === 'session.status_idle').length >= 2,
  ...
  it('无效确认:未知 id / 同批重复 / 已裁决,均 400', async () => {
    ...
    const unknown = await sendEvents(session.id, [
      {
        type: 'user.tool_confirmation',
        tool_use_id: 'sevt_not_pending',
        result: 'allow',
      },
    ]);
    expect(unknown.status).toBe(400);
    expect((await jsonBody<ErrorEnvelope>(unknown)).error.message).toContain(
    ...
    expect((await jsonBody<ErrorEnvelope>(duplicate)).error.message).toContain(
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'more please ask-invalid' }],
      },
    ]);
    ...
    const ids = (
      second.filter((event) => event.type === 'session.status_idle')[1]!.stop_reason as {
        event_ids: string[];
      }
    ).event_ids;
    await sendEvents(session.id, [
    ...
    expect((await jsonBody<ErrorEnvelope>(reconfirm)).error.message).toContain(
  ...
  it('逐出恢复重入确认链:裁决已持久化,重执行未完成的工具,不重复落结果', async () => {
    ...
    await enqueueModelScript({
      ...
      tool_calls: [
        {
          name: 'bash',
          arguments: JSON.stringify({ command: 'slow-confirm' }),
        },
      ],
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'please ask-evict' }],
      },
    ]);
    const suspended = await pollForEvent(session.id, (list) =>
      list.some((event) => event.type === 'session.status_idle'),
    ...
    expect((results[0]!.content as Array<{ text: string }>)[0]!.text).toContain(
      'slow command finished',
    );
...
describe('M3 running 中追加消息(§4.4 收尾竞态)', () => {
  it('running 时追加的消息不打断当前请求,在后续迭代被消费', async () => {
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'please backlog-probe one' }],
      },
    ]);
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'backlog-probe two' }],
      },
    ]);

    const events = await pollForEvent(session.id, (list) =>
      list.some((event) => event.type === 'session.status_idle'),
    );
    ...
    expect(
      events.every((event) => event.type !== 'user.message' || event.processed_at !== null),
    ).toBe(true);
  });
});
