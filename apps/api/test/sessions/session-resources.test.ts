...
describe("POST /v1/sessions/{sessionId}/resources 挂载", () => {
  it("省略 mount_path 得默认路径;相对路径归一化回显;响应 201", async () => {
    ...
    const defaulted = await jsonBody<FileResourceJson>(await postResource(session.id, { type: "file", file_id: file.id }));
    ...
    const named = await jsonBody<FileResourceJson>(
      await postResource(session.id, { type: "file", file_id: file.id, mount_path: "datasets/q2.csv" }),
    );
  ...
  it("逃逸、重叠、file 不存在、非法类型分别 400", async () => {
    ...
    const escape = await postResource(session.id, { type: "file", file_id: other.id, mount_path: "../up" });
    expect(escape.status).toBe(400);

    // 与既有挂载 /mnt/session/uploads/raw/a.csv 前缀重叠(挂到 raw 目录本身)
    const overlap = await postResource(session.id, { type: "file", file_id: other.id, mount_path: "raw" });
    expect(overlap.status).toBe(400);
    expect((await jsonBody<ErrorEnvelope>(overlap)).error.message).toContain("overlaps");

    // 兄弟路径(同目录下的另一个文件)不算重叠,允许
    const sibling = await postResource(session.id, { type: "file", file_id: other.id, mount_path: "raw/inner.csv" });
    expect(sibling.status).toBe(201);

    // 完全相同路径也被拒(服务层检查)
    const duplicate = await postResource(session.id, { type: "file", file_id: other.id, mount_path: "raw/a.csv" });
    expect(duplicate.status).toBe(400);

    const missing = await postResource(session.id, { type: "file", file_id: "file_00000000-0000-7000-8000-000000000000" });
    expect(missing.status).toBe(400);

    const wrongType = await postResource(session.id, { type: "memory_store", memory_store_id: "ms_1" });
  ...
  it("会话不存在 404;已归档会话挂载 409", async () => {
    expect(
      (await postResource("sess_00000000-0000-7000-8000-000000000000", { type: "file", file_id: "file_x" }))
        .status,
    ).toBe(404);
    ...
    const res = await postResource(session.id, { type: "file", file_id: file.id });
...
describe("GET /v1/sessions/{sessionId}/resources 列表与单查", () => {
  it("分页列出挂载资源,游标翻页", async () => {
    ...
    const page1 = await jsonBody<PageJson<FileResourceJson>>(await listResources(session.id, "?limit=1"));
  ...
  it("归档会话仍可列出资源;单查命中与 404", async () => {
    ...
    expect((await getResource(session.id, "sres_00000000-0000-7000-8000-000000000000")).status).toBe(404);
...
describe("DELETE /v1/sessions/{sessionId}/resources/{resourceId} 卸载", () => {
  it("卸载成功返回回执,重复卸载 404,File 不受影响", async () => {
    ...
    expect(await jsonBody(res)).toEqual({ id: target.id, type: "session_resource_deleted" });
