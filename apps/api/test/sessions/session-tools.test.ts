...
async function _eventList(sessionId: string): Promise<EventJson[]> {
...
describe('M2 工具循环 — always_allow 闭环', () => {
  it('bash 工具往返:tool_use/tool_result 配对(事件 id 即 tool_use_id),usage 跨迭代累计', async () => {
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'please use-bash now' }],
      },
    ]);
  ...
  it('一次响应多个工具调用:逐个执行按序配对', async () => {
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'please multi-tools' }],
      },
    ]);
    const events = await pollForEvent(session.id, (list) =>
      list.some((event) => event.type === 'session.status_idle'),
    ...
    expect(toolResults.map((event) => event.tool_use_id)).toEqual(
      toolUses.map((event) => event.id),
    );
  ...
  it('非法入参与未知工具:is_error 的 tool_result 喂回模型,turn 继续', async () => {
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'please bad-input' }],
      },
    ]);
    const events = await pollForEvent(session.id, (list) =>
      list.some((event) => event.type === 'session.status_idle'),
  ...
  it('always_ask 工具随 tools 一同上行,执行时才分叉挂起(M3)', async () => {
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'ask-policy probe' }],
      },
    ]);
...
describe('M2 工具循环 — 中断与恢复', () => {
  it('中断在途:执行完再停,未开始的合成 is_error 结果补齐配对', async () => {
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'please interrupt-tool' }],
      },
    ]);
  ...
  it('逐出后恢复:重入工具批次,重执行未完成的工具,终事件无重复', async () => {
    ...
    await sendEvents(session.id, [
      {
        type: 'user.message',
        content: [{ type: 'text', text: 'please recover-tools' }],
      },
    ]);
