...
describe('DELETE /v1/sessions/{sessionId}', () => {
  ...
  it('删除级联清理产出编目:映射行、File 行与 R2 对象一起清', async () => {
    ...
    const output = await env.DB.prepare('SELECT file_id FROM session_outputs WHERE session_id = ?')
      .bind(created.id)
  ...
  it('不存在的 id 返回 404', async () => {
    expect((await deleteSessionViaApi('sess_00000000-0000-7000-8000-000000000000')).status).toBe(
      404,
    );
  });
});
