...
describe('POST /v1/sessions/{sessionId}/archive', () => {
  ...
  it('归档后:列表默认不出现、include_archived=true 出现、更新被拒(端到端回归)', async () => {
    ...
    const visibleRes = await listSessions(
      `?agent_id=${created.agent.id}&limit=100&include_archived=true`,
    );
  ...
  it('不存在的 id 返回 404', async () => {
    expect((await archiveSessionViaApi('sess_00000000-0000-7000-8000-000000000000')).status).toBe(
      404,
    );
  });
});
