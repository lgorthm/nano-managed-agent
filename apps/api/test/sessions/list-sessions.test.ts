...
describe("GET /v1/sessions 列表与分页", () => {
  it("按 agent_id 过滤,默认按创建时间倒序,游标翻页", async () => {
    ...
    const second = await listSessions(`?agent_id=${agent.id}&limit=2&page=${encodeURIComponent(page1.next_page!)}`);
...
describe("GET /v1/sessions 过滤参数", () => {
  it("agent_id + agent_version 过滤匹配钉住的版本;version 单独出现 400", async () => {
    ...
    // 钉 v1 与钉 v2 各一个会话
    const pinnedV1 = await createDefaultSession({ agent: { type: "agent", id: agent.id, version: 1 } });
  ...
  it("created_at 区间过滤,非法时间 400", async () => {
    ...
    const hit = await jsonBody<PageJson<SessionJson>>(
      await listSessions(`?agent_id=${agent.id}&created_at[gte]=${encodeURIComponent(before)}&created_at[lt]=${encodeURIComponent(after)}`)
    );
