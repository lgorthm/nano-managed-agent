...
describe("POST /v1/sessions/{sessionId} 更新规则", () => {
  it("title 替换与 null 清空", async () => {
    const created = await createDefaultSession({ title: "old title", metadata: { keep: "me" } });

    const renamed = await jsonBody<SessionJson>(await updateSession(created.id, { title: "new title" }));
  ...
  it("metadata 按键合并:覆盖、删键、保留未提及键", async () => {
    const created = await createDefaultSession({ metadata: { a: "1", b: "2" } });
    const body = await jsonBody<SessionJson>(
      await updateSession(created.id, { metadata: { b: "two", c: "3", a: null } }),
    );
  ...
  it("无变化更新不写库,updated_at 不变", async () => {
    const created = await createDefaultSession({ title: "same", metadata: { k: "v" } });
    const body = await jsonBody<SessionJson>(
...
describe("POST /v1/sessions/{sessionId} 409 与 404", () => {
  ...
  it("running 会话更新 agent.tools 返回 409(session_not_idle);title 不受限", async () => {
    ...
    const title = await jsonBody<SessionJson>(await updateSession(created.id, { title: "still ok" }));
