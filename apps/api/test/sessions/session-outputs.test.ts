...
import { type HarvestResult, harvestSessionOutputs } from '../../src/runtime/tools/catalog';
import {
  deleteFile,
  downloadFile,
  type FileJson,
...
describe('harvestSessionOutputs(收割编目)', () => {
  ...
  it('差集:沙箱里消失的产出连行带对象清理', async () => {
    ...
    const goneFileId = (await outputRows(session.id)).find(
      (row) => row.path === 'gone.txt',
    )!.file_id;
...
describe('会话产出的 File wire 语义', () => {
  async function sessionWithOutput(
    content = 'payload',
  ): Promise<{ session: SessionJson; fileId: string }> {
    const session = await createDefaultSession();
  ...
  it('scope_id 过滤返回挂载 ∪ 产出,产出回显 scope', async () => {
    ...
    expect((await postResource(session.id, { type: 'file', file_id: uploaded.id })).status).toBe(
      201,
    );
  ...
  it('全局列表:产出恒回显 scope,上传文件不回显', async () => {
    ...
    expect(byId.get(fileId)?.scope).toEqual({
      type: 'session',
      id: session.id,
    });
  ...
  it('get-file 与下载端点:产出文件可点查、可下载且内容一致', async () => {
    ...
    const meta = await jsonBody<FileJson & { scope?: { type: string; id: string } }>(
