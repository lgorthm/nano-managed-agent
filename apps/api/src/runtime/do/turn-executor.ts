...
import { type ChatModelConfig, ModelAbortedError, streamChatCompletion } from '../model-client';
  ...
  for (;;) {
    ...
    if (snapshot.pendingToolCalls.length > 0) {
      if (runner === null) {
        ...
        host.appendProducedEvent('session.error', {
          message: 'Tool runner is unavailable.',
        });
          ...
          host.appendProducedEvent(
            'agent.tool_result',
            {
              ...
              content: [
                {
                  type: 'text',
                  text: 'Tool execution was skipped because the session was interrupted.',
                },
              ],
              is_error: true,
            },
      ...
      if (askCalls.length > 0) {
        ...
        await host.finishTurn(
          turnId,
          {
            ...
            event_ids: askCalls.map((call) => call.toolUseEventId),
          },
          totalUsage,
        );
        return;
    ...
    const messages: ChatMessage[] = assembleChatMessages({
    ...
    const deltaBuffer: Record<'thinking' | 'message', string> = {
      thinking: '',
      message: '',
    };
      ...
      const result = await streamChatCompletion(
        ...
        messages,
        {
          ...
          onDelta: (kind, text) => {
            ...
            chunkCount += 1;
            if (
              chunkCount % SNAPSHOT_CHUNK_INTERVAL === 0 ||
              Date.now() - lastSnapshotAt >= SNAPSHOT_TIME_INTERVAL_MS
            ) {
              host.saveTurnSnapshot(turnId, iteration, snapshot);
