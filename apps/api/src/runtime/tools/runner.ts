...
export class MockToolRunner implements ToolRunner {
  ...
  async run(invocation: ToolInvocation): Promise<ToolOutcome> {
    ...
    if (invocation.name === "bash" && typeof input.command === "string" && input.command.startsWith("slow")) {
      await new Promise((resolve) => setTimeout(resolve, 1500));
      return { content: `slow command finished: ${input.command}`, isError: false };
    }
    return { content: `mock:${invocation.name}:${JSON.stringify(input)}`, isError: false };
  }
...
class UnavailableToolRunner implements ToolRunner {
  ...
  async run(): Promise<ToolOutcome> {
    return { content: "Tool execution is unavailable: no sandbox binding is configured.", isError: true };
  }
  async harvestOutputs(): Promise<void> {}
  async destroy(): Promise<void> {}
}
...
export class SandboxToolRunner implements ToolRunner {
  ...
  private sbx(): Sandbox {
    if (this.sandbox === null) {
      ...
      this.sandbox = getSandbox(this.env.SANDBOX!, this.ctx.sessionId, { sleepAfter: "10m" });
    }
    return this.sandbox;
  }

  private exec(command: string, timeoutMs?: number) {
    return this.sbx().exec(command, { cwd: WORKSPACE, timeout: timeoutMs, origin: "internal" });
  }
  ...
  private async ensureMaterialized(): Promise<void> {
    ...
    for (const resource of this.ctx.resources) {
      ...
      await this.sbx().writeFile(resource.mountPath, bytesToBase64(bytes), { encoding: "base64" });
    }
    ...
    for (const skill of this.ctx.skills) {
      ...
      for (const file of skill.files) {
        await this.sbx().writeFile(`${dir}/${file.path}`, bytesToBase64(file.content), { encoding: "base64" });
      }
    }
  ...
  private async applyPackages(packages: NormalizedEnvironmentPackages): Promise<void> {
    ...
    const attempts: Array<readonly [boolean, string]> = [
      [packages.apt.length > 0, `(command -v apt-get >/dev/null && apt-get update -qq && DEBIAN_FRONTEND=noninteractive apt-get install -y -qq ${join(packages.apt)}) || echo 'apt unavailable'`],
      [packages.pip.length > 0, `(command -v pip3 >/dev/null && pip3 install -q --break-system-packages ${join(packages.pip)}) || (command -v pip3 >/dev/null && pip3 install -q ${join(packages.pip)}) || echo 'pip unavailable'`],
      [packages.npm.length > 0, `(command -v npm >/dev/null && npm install -g ${join(packages.npm)}) || echo 'npm unavailable'`],
      [packages.go.length > 0, `(command -v go >/dev/null && go install ${join(packages.go.map((pkg) => `${pkg}@latest`))}) || echo 'go unavailable'`],
      [packages.cargo.length > 0, `(command -v cargo >/dev/null && cargo install ${join(packages.cargo)}) || echo 'cargo unavailable'`],
      [packages.gem.length > 0, `(command -v gem >/dev/null && gem install ${join(packages.gem)}) || echo 'gem unavailable'`],
    ];
  ...
  async run(invocation: ToolInvocation): Promise<ToolOutcome> {
    try {
      ...
      if (!parsed.success) {
        ...
        return {
          content: `Invalid arguments for ${invocation.name}: ${issue?.path.join(".") ?? ""} ${issue?.message ?? ""}`.trim(),
          isError: true,
        };
      }
      ...
      switch (invocation.name as BuiltinToolName) {
        case "bash": {
          ...
          return { content: truncateToolOutput(content), isError: !result.success };
        }
        case "read": {
          const file = await this.sbx().readFile(resolvePath(String(input.path)), { encoding: "utf-8" });
        ...
        case "edit": {
          ...
          if (count > 1) {
            return { content: `old_string occurs ${count} times in ${input.path}; it must be unique.`, isError: true };
          }
          await this.sbx().writeFile(path, file.content.replace(oldString, String(input.new_string)));
          return { content: `Edited ${input.path}.`, isError: false };
        }
        case "grep": {
          const target = shellQuote(input.path !== undefined ? resolvePath(String(input.path)) : WORKSPACE);
          // grep exit 1 = 无匹配,是正常结果而非错误
          const result = await this.exec(`grep -rnE --include='*' -- ${shellQuote(String(input.pattern))} ${target}`);
          return { content: truncateToolOutput(result.stdout === "" ? "(no matches)" : result.stdout), isError: false };
        }
        case "find": {
          const name = input.pattern !== undefined ? ` -name ${shellQuote(String(input.pattern))}` : "";
          const result = await this.exec(`find ${shellQuote(input.path !== undefined ? resolvePath(String(input.path)) : WORKSPACE)} -type f${name}`);
          return { content: truncateToolOutput(result.stdout), isError: false };
        }
        case "ls": {
          const result = await this.exec(`ls -la ${shellQuote(input.path !== undefined ? resolvePath(String(input.path)) : WORKSPACE)}`);
          return { content: truncateToolOutput(result.stdout), isError: false };
        }
      }
    } catch (err) {
      console.error(`tool ${invocation.name} failed:`, err);
      return { content: `Tool execution failed: ${err instanceof Error ? err.message : String(err)}`, isError: true };
    }
  }
