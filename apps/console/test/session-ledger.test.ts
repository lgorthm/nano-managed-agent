...
function sampleEvents(): Array<Record<string, unknown>> {
  ...
  return [
    ev('user.message', {
      processed_at: at(0),
      content: [{ type: 'text', text: '帮我看看这个仓库' }],
    }),
    ...
    ev('agent.message', {
      processed_at: at(220),
      content: [{ type: 'text', text: '我来列一下目录' }],
    }),
    ev('agent.tool_use', {
      processed_at: at(300),
      name: 'bash',
...
describe('deriveTimelineSpans', () => {
  ...
  it('duration 模式:压缩空闲间隙,连续块紧贴铺开', () => {
    ...
    const records = buildLedger([
      ...
      ev('agent.message', {
        processed_at: at(10_000),
        content: [{ type: 'text', text: '远处的消息' }],
      }),
    ]);
    ...
    expect(byKey.get('sevt_4')).toMatchObject({
      start: BASE + 100,
      end: BASE + 100,
    });
  ...
  it('无可定位记录(全部排队中)时返回 null', () => {
    resetSeq();
    const records = buildLedger([
      ev('user.message', { content: [{ type: 'text', text: '排队' }] }),
    ]);
