...
describe('URL 改写', () => {
  it('/glm/<rest> 映射到 GLM API 并保留查询串', () => {
    const upstream = buildUpstreamUrl(
      new URL('http://localhost/glm/agent/managed/v1/agents?limit=5&order=desc'),
    );
    expect(upstream.href).toBe(
      'https://agent-api.bigmodel.cn/api/agent/managed/v1/agents?limit=5&order=desc',
    );
  });
...
describe('上游请求构造', () => {
  it('注入鉴权与协议头,剔除本站 Cookie 与 CF 头', () => {
    ...
    const upstream = buildUpstreamRequest(
      request,
      buildUpstreamUrl(new URL(request.url)),
      'test-key',
    );
...
describe('nano 分支', () => {
  ...
  it('files 列表:after_id 映射为 page,响应适配出 has_more/first_id/last_id', async () => {
    ...
    const upstreamPage = new Response(
      JSON.stringify({
        data: [{ id: 'file_2' }],
        next_page: 'eyJraW5kIjoiZmlsZXMifQ',
      }),
  ...
  it('files 列表末页:next_page 为 null 时 has_more 为 false、last_id 为 null', async () => {
    const upstreamPage = new Response(
      JSON.stringify({ data: [{ id: 'file_1' }], next_page: null }),
      {
        status: 200,
        headers: { 'content-type': 'application/json' },
      },
