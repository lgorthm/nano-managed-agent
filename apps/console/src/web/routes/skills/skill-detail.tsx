    ...
    <Dialog open={open} onOpenChange={mutation.isPending ? undefined : onOpenChange}>
      ...
      <DialogContent showCloseButton={false} className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            所有版本与文件一并删除,不可恢复。被未归档 Agent
            的活动配置引用时删除会被拒绝,需先解除引用或归档 Agent。
          </DialogDescription>
        </DialogHeader>
        {mutation.isError ? (
    ...
    <Dialog open={open} onOpenChange={mutation.isPending ? undefined : setOpen}>
      ...
      <DialogContent showCloseButton={false} className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            删除最新版本时,latest_version 会回落到剩余的最大版本;没有剩余版本时 Skill
            变成空壳,可继续上传新版本。
          </DialogDescription>
        </DialogHeader>
        {mutation.isError ? (
...
export function SkillDetailPage() {
  ...
  const latest = skill.latest_version
    ? versions.find((v) => v.version === skill.latest_version)
    : undefined;
    ...
    <div className="flex flex-1 flex-col gap-4">
      ...
      <PageHeader
        ...
        actions={
          <>
            <RefreshButton
              isFetching={skillQuery.isFetching || versionsQuery.isFetching}
              onClick={() => {
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <SectionCard title="最新版本描述" contentClassName="max-h-64 overflow-auto">
          {latest ? (
                  ...
                  <TableRow key={version.version}>
                    <TableCell className="font-mono text-xs tabular-nums">
                      ...
                      {version.version === skill.latest_version ? (
                        <Badge
                          variant="secondary"
                          className="ml-1.5 px-1.5 text-[10px] font-normal"
                        >
                          最新
                        </Badge>
                      ) : null}
