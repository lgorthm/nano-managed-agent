...
function DeploymentFormFields({
  ...
  return (
    <div className="grid gap-4">
        ...
        <>
          ...
          <div className="grid gap-2">
            ...
            <Select
              ...
              onValueChange={(v) => onChange({ environmentId: v })}
            >
              <SelectTrigger className="w-full">
                <SelectValue
                  placeholder={
                    environments.length === 0 ? '暂无可选 Environment' : '选择 Environment'
                  }
                />
              </SelectTrigger>
              <SelectContent>
      ...
      <div className="grid gap-2">
        <Label htmlFor={`${idPrefix}-message`}>初始消息{isCreate ? '' : '(留空 = 保持不变)'}</Label>
        <Textarea
          id={`${idPrefix}-message`}
          rows={4}
        ...
        initial_events: [
          {
            ...
            content: [{ type: 'text' as const, text: form.initialMessage.trim() }],
          },
        ],
...
export function UpdateDeploymentDialog({ deployment }: { deployment: Deployment }) {
  const existingMessage = deployment.initial_events
    .map((event) =>
      event.content.map((block) => (block.type === 'text' ? block.text : '')).join('\n'),
    )
    .join('\n');
  ...
  const error =
    validateForm(form, false) ?? (choice === 'cron' ? validateCron(form.cronExpression) : null);
    ...
    () => {
      ...
      if (form.initialMessage.trim() && form.initialMessage.trim() !== existingMessage.trim()) {
        input.initial_events = [
          {
            type: 'user.message',
            content: [{ type: 'text', text: form.initialMessage.trim() }],
          },
        ];
      }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[85svh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            名称、描述、调度与初始消息可更新;Agent 固定与 Environment
            绑定需要在更新接口中显式指定,控制台暂不改动。
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4">
          <div className="grid gap-2">
