...
const TOOL_DESCRIPTIONS: Record<BuiltinToolName, string> = {
  bash: 'Run a bash command in the session sandbox. The working directory persists between calls within the session. Use this for building, running scripts, and any shell work.',
  ...
  edit: 'Edit a file by replacing an exact string. The file must exist and old_string must match the text to replace exactly once.',
  grep: 'Search file contents with a regular expression (extended regex, like egrep) under a path. Returns matching lines with file names and line numbers.',
...
const TOOL_PARAMETERS: Record<BuiltinToolName, Record<string, unknown>> = {
  ...
  find: {
    ...
    properties: {
      pattern: {
        type: 'string',
        description: "Name glob like '*.md'; defaults to all files.",
      },
      path: {
        type: 'string',
...
/** 模型侧定义表(bash 排首位,最常用) */
export const BUILTIN_TOOL_DEFINITIONS: Record<BuiltinToolName, ChatToolDefinition> =
  Object.fromEntries(
...
export function resolveBuiltinTools(
  toolsets: readonly NormalizedAgentToolset[],
): ResolvedBuiltinTool[] {
