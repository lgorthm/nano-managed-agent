...
describe('SessionCreateRequestSchema 字段约束与一期裁剪', () => {
  it('environment_id 形态校验:非 env_ UUID 拒绝', () => {
    expect(
      SessionCreateRequestSchema.safeParse({
        agent: AGENT_ID,
        environment_id: 'sess_wrong',
      }).success,
    ).toBe(false);
    expect(
      SessionCreateRequestSchema.safeParse({
        agent: AGENT_ID,
        environment_id: `ENV_${'0'.repeat(36)}`,
      }).success,
    ).toBe(false);
  });
  ...
  it('resources 超 508 拒绝;超 500 file 拒绝', () => {
    const many = Array.from({ length: 509 }, () => ({
      type: 'file',
      file_id: FILE_ID,
    }));
    expect(
    ...
    const fiveHundredAndOne = Array.from({ length: 501 }, () => ({
      type: 'file',
      file_id: FILE_ID,
    }));
    expect(
  ...
  it('title / metadata / agent.tools 各自单独成立', () => {
    ...
    expect(
      SessionUpdateRequestSchema.safeParse({
        agent: { tools: [{ type: 'agent_toolset_20260601' }] },
      }).success,
    ).toBe(true);
  });
