...
describe("内置工具定义与入参校验", () => {
  ...
  it("入参校验拒绝未知键与类型错误(模型生成物不可信)", () => {
    expect(BUILTIN_TOOL_INPUT_SCHEMAS.bash.safeParse({ command: "ls", extra: 1 }).success).toBe(false);
    ...
    expect(BUILTIN_TOOL_INPUT_SCHEMAS.edit.safeParse({ path: "a", old_string: "x" }).success).toBe(false);
  });
});
...
describe("validateToolInvocation(执行前的协议级校验)", () => {
  it("未知工具与非法 JSON 入参(标记对象)被拒绝,合法入参通过", () => {
    ...
    expect(validateToolInvocation("bash", { __invalid_json: "{broken" })).toContain("Invalid arguments");
    expect(validateToolInvocation("bash", { command: "ls" })).toBeNull();
    expect(validateToolInvocation("edit", { path: "a", old_string: "x", new_string: "y" })).toBeNull();
  });
});
