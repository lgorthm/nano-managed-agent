...
describe('assembleChatMessages 事件映射', () => {
  ...
  it('未消费输入(processed_at null)同样进入上下文', () => {
    const pending = event('user.message', {
      ...
      events: [
        event('user.message', {
          content: [
            {
              type: 'document',
              source: { type: 'text', media_type: 'text/plain', data: '正文' },
              title: '说明',
            },
    ...
    const messages = assembleChatMessages({
      ...
      events: [
        ...
        event('agent.tool_result', {
          tool_use_id: 'sevt_agent.tool_use',
          content: [{ type: 'text', text: 'file-a' }],
        }),
        event('agent.message', {
          content: [{ type: 'text', text: '两个文件' }],
        }),
      ],
    });
    expect(messages).toEqual([
