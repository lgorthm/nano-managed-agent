...
describe("normalizeMountPath 默认与拼接", () => {
  it("省略与 null 都得到默认路径", () => {
    ...
    expect(normalizeMountPath(undefined, FILE_ID)).toEqual({ ok: true, path: expected });
    expect(normalizeMountPath(null, FILE_ID)).toEqual({ ok: true, path: expected });
  });
...
describe("normalizeMountPath 非法输入", () => {
  ...
  it(".. 越过根即逃逸,拒绝", () => {
    expect(normalizeMountPath("../escape", FILE_ID)).toMatchObject({ ok: false });
    expect(normalizeMountPath("/../../escape", FILE_ID)).toMatchObject({ ok: false });
    expect(normalizeMountPath("a/../../escape", FILE_ID)).toMatchObject({ ok: false });
  });
...
describe("mountPathsOverlap 重叠判定", () => {
  it("相等路径重叠", () => {
    expect(mountPathsOverlap(`${SESSION_UPLOAD_ROOT}/a.csv`, `${SESSION_UPLOAD_ROOT}/a.csv`)).toBe(true);
  });

  it("目录与其中文件互为前缀,重叠", () => {
    expect(mountPathsOverlap(`${SESSION_UPLOAD_ROOT}/data`, `${SESSION_UPLOAD_ROOT}/data/a.csv`)).toBe(true);
    expect(mountPathsOverlap(`${SESSION_UPLOAD_ROOT}/data/a.csv`, `${SESSION_UPLOAD_ROOT}/data`)).toBe(true);
  });

  it("兄弟路径不重叠", () => {
    expect(mountPathsOverlap(`${SESSION_UPLOAD_ROOT}/data-a`, `${SESSION_UPLOAD_ROOT}/data`)).toBe(false);
    expect(mountPathsOverlap(`${SESSION_UPLOAD_ROOT}/data/a.csv`, `${SESSION_UPLOAD_ROOT}/data/b.csv`)).toBe(false);
  });

  it("同文件名不同目录不重叠", () => {
    expect(mountPathsOverlap(`${SESSION_UPLOAD_ROOT}/x/a.csv`, `${SESSION_UPLOAD_ROOT}/y/a.csv`)).toBe(false);
  });
