...
describe('resolveSessionAgent 覆盖语义', () => {
  ...
  it('model 对象形态保留显式 effort', () => {
    ...
    const resolved = resolveSessionAgent(
      base,
      overridesOf({ model: { id: 'glm-5.3', effort: 'low' } }),
    );
  ...
  it('tools 整体替换并归一化(default_config 补全);null 与空数组等价清空', () => {
    ...
    const replaced = resolveSessionAgent(
      base,
      overridesOf({ tools: [{ type: 'agent_toolset_20260601' }] }),
    );
  ...
  it('skills 与 mcp_servers 整体替换;null 清空;省略继承', () => {
    ...
    expect(resolveSessionAgent(base, overridesOf({ mcp_servers: [server] })).mcp_servers).toEqual([
      server,
    ]);
  });
