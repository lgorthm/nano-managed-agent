...
describe("mergeEnvironmentRecord 更新语义", () => {
  ...
  it("name 替换;description 传 null 清空;省略均保持不变", () => {
    const merged = mergeEnvironmentRecord(current(), parseUpdate({ name: "renamed", description: null }));
  ...
  it("config 整体替换(非深合并):未提及的包管理器列表清空", () => {
    const merged = mergeEnvironmentRecord(
      current(),
      parseUpdate({ config: { type: "cloud", packages: { pip: ["requests"] } } }),
    );
...
describe("environmentRecordEquals 无变化检测", () => {
  ...
  it("任何一处不同则为假", () => {
    const state = current();
    expect(environmentRecordEquals(mergeEnvironmentRecord(state, parseUpdate({ name: "x" })), state)).toBe(false);
    expect(
      environmentRecordEquals(
    ...
    expect(environmentRecordEquals(mergeEnvironmentRecord(state, parseUpdate({ metadata: { env: null } })), state)).toBe(
      false,
    );
  });
...
describe("联动校验作用于合并后的完整配置", () => {
  ...
  it("只提交 networking 的 config 会整体替换,packages 清空后 limited 无需放行", () => {
    ...
    const merged = mergeEnvironmentRecord(
      current(),
      parseUpdate({ config: { type: "cloud", networking: { type: "limited", allowed_hosts: ["api.example.com"] } } }),
    );
