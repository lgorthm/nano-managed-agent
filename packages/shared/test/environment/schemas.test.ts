...
describe('config 校验', () => {
  ...
  it('packages 内未知字段(自定义 registry 配置)被拒绝', () => {
    expect(
      withConfig({
        type: 'cloud',
        packages: { pip: ['requests'], index_url: 'https://pypi.example.com' },
      }).success,
    ).toBe(false);
  });
...
describe('网络策略校验', () => {
  ...
  it('allowed_hosts 带协议 / 端口 / 路径被拒绝', () => {
    ...
    expect(withNetworking({ type: 'limited', allowed_hosts: ['api.example.com/v1'] }).success).toBe(
      false,
    );
  });

  it('通配只接受 *. 前缀的整标签形式', () => {
    expect(withNetworking({ type: 'limited', allowed_hosts: ['*.example.com'] }).success).toBe(
      true,
    );
    expect(withNetworking({ type: 'limited', allowed_hosts: ['*example.com'] }).success).toBe(
      false,
  ...
  it('非法标签形态被拒绝', () => {
    expect(withNetworking({ type: 'limited', allowed_hosts: ['-bad.example.com'] }).success).toBe(
      false,
    );
    expect(withNetworking({ type: 'limited', allowed_hosts: ['bad-.example.com'] }).success).toBe(
      false,
...
describe('EnvironmentUpdateRequestSchema', () => {
  ...
  it('config 可为 null(恢复默认),config.type 仍必填', () => {
    ...
    expect(
      EnvironmentUpdateRequestSchema.safeParse({
        config: { packages: { pip: ['x'] } },
      }).success,
    ).toBe(false);
    expect(EnvironmentUpdateRequestSchema.safeParse({ config: { type: 'cloud' } }).success).toBe(
      true,
    );
  });
