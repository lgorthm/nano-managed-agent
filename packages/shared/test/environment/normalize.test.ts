...
import { EnvironmentCreateRequestSchema, normalizeEnvironmentConfig, normalizeEnvironmentCreate } from "../../src";
...
describe("normalizeEnvironmentConfig 默认值展开", () => {
  ...
  it("packages 省略或 null 规范化为六键空数组", () => {
    expect(normalize({ name: "a", config: { type: "cloud" } }).config.packages).toEqual(EMPTY_PACKAGES);
    expect(normalize({ name: "a", config: { type: "cloud", packages: { pip: ["requests"] } } }).config.packages).toEqual({
      ...EMPTY_PACKAGES,
      pip: ["requests"],
    });
  });
  ...
  it("networking 省略或 null 规范化为 unrestricted", () => {
    expect(normalize({ name: "a", config: { type: "cloud" } }).config.networking).toEqual({ type: "unrestricted" });
  });
});
...
describe("包列表去重(保留首次出现顺序)", () => {
  ...
  it("六类管理器独立处理,未声明的为空数组", () => {
    const { config } = normalize({
      name: "a",
      config: { type: "cloud", packages: { apt: ["poppler-utils", "poppler-utils"], npm: ["typescript"] } },
    });
  ...
  it("小写化 + 排序 + 去重", () => {
    const { config } = normalize({
      ...
      config: {
        ...
        networking: {
          type: "limited",
          allowed_hosts: ["Zulu.example.com", "alpha.example.com", "zulu.example.com", "*.Internal.example.com"],
        },
      },
    });
...
describe("联网开关补全", () => {
  ...
  it("显式值保留", () => {
    const { config } = normalize({
      ...
      config: {
        type: "cloud",
        networking: { type: "limited", allow_package_managers: true, allow_mcp_servers: null },
      },
    });
    expect(config.networking).toEqual({
      type: "limited",
...
describe("normalizeEnvironmentCreate 记录级字段", () => {
  ...
  it("description 与 metadata 透传", () => {
    const record = normalize({ name: "a", description: "d", metadata: { team: "infra" } });
