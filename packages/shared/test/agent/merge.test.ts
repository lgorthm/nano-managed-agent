...
describe('mergeAgentConfig 标量字段', () => {
  ...
  it('model 对象形态保留显式 effort', () => {
    expect(merge(currentConfig(), { model: { id: 'glm-5.3', effort: 'low' } }).model.effort).toBe(
      'low',
    );
  });
...
describe('mergeAgentConfig metadata 按键合并', () => {
  it('新键新增、旧键覆盖、null 删键、未提及键保留', () => {
    ...
    expect(merge(current, { metadata: { b: 'two', c: '3' } }).metadata).toEqual({
      a: '1',
      b: 'two',
      c: '3',
    });
    expect(merge(current, { metadata: { a: null } }).metadata).toEqual({
      b: '2',
    });
    expect(merge(current, {}).metadata).toEqual({ a: '1', b: '2' });
    expect(merge(current, { metadata: null }).metadata).toEqual({
      a: '1',
      b: '2',
    });
  });
...
describe('agentConfigEquals 无变化检测', () => {
  ...
  it('metadata 键值变化为假', () => {
    expect(
      agentConfigEquals(
        currentConfig({ metadata: { k: '1' } }),
        currentConfig({ metadata: { k: '2' } }),
      ),
    ).toBe(false);
  });
});
