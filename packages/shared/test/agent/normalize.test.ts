...
function asBuiltinToolset(toolset: NormalizedAgentToolset | undefined): NormalizedBuiltinToolset {
  if (!toolset || toolset.type !== "agent_toolset_20260601") {
...
describe("normalizeAgentConfig 模型归一化", () => {
  ...
  it("对象形态显式 effort 保留", () => {
    expect(normalize({ name: "a", model: { id: "glm-5.3", effort: "low" } }).model.effort).toBe("low");
  });

  it("对象形态 effort 为 null 时取默认", () => {
    expect(normalize({ name: "a", model: { id: "glm-5.3", effort: null } }).model.effort).toBe("max");
  });

  it("speed 省略或为 null 时补为 standard", () => {
    expect(normalize({ name: "a", model: { id: "glm-5.3" } }).model.speed).toBe("standard");
    expect(normalize({ name: "a", model: { id: "glm-5.3", speed: null } }).model.speed).toBe("standard");
  });
});
...
describe("normalizeAgentConfig 工具集归一化", () => {
  ...
  it("default_config 省略时补为 enabled=true、always_allow", () => {
    const config = normalize({ name: "a", model: "glm-5.3", tools: [{ type: "agent_toolset_20260601" }] });
  ...
  it("configs 覆盖 default_config 的字段", () => {
    const config = normalize({
      ...
      tools: [
        {
          type: "agent_toolset_20260601",
          configs: [{ name: "edit", enabled: false, permission_policy: { type: "always_ask" } }],
        },
      ],
    });
    const toolset = asBuiltinToolset(config.tools[0]);
    expect(toolset.default_config).toEqual({ enabled: true, permission_policy: { type: "always_allow" } });
    expect(toolset.configs).toEqual([
      ...
      tools: [
        {
          ...
          configs: [{ name: "read", enabled: null, permission_policy: { type: "always_ask" } }],
        },
      ],
    });
    const toolset = asBuiltinToolset(config.tools[0]);
    expect(toolset.configs).toEqual([
      { name: "read", enabled: false, permission_policy: { type: "always_ask" } },
    ]);
  });
  ...
  it("mcp_toolset 保留 mcp_server_name 并归一化配置", () => {
    const config = normalize({
      name: "a",
      model: "glm-5.3",
      tools: [{ type: "mcp_toolset", mcp_server_name: "kb", configs: [{ name: "search" }] }],
  ...
  it("custom 工具原样透传", () => {
    const inputSchema = { type: "object", properties: { q: { type: "string" } }, required: ["q"] } as const;
    const config = normalize({
      name: "a",
      model: "glm-5.3",
      tools: [{ type: "custom", name: "lookup_order", description: "查询订单", input_schema: inputSchema }],
    });
    expect(config.tools[0]).toEqual({
      type: "custom",
      name: "lookup_order",
...
describe("normalizeAgentConfig 标量与元数据", () => {
  ...
  it("metadata 省略时归一化为空对象,提供时原样保留", () => {
    ...
    expect(normalize({ name: "a", model: "glm-5.3", metadata: { team: "infra" } }).metadata).toEqual({
      team: "infra",
    });
  });
});
