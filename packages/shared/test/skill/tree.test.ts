...
describe('normalizeSkillTree 路径校验', () => {
  ...
  it('重复路径被拒绝', async () => {
    ...
    expect(result).toMatchObject({
      ok: false,
      error: { code: 'duplicate_path', param: 'SKILL.md' },
    });
  });
});
...
describe('normalizeSkillTree 上限裁决', () => {
  it('文件数恰好等于上限时通过,超出报 too_many_files', async () => {
    ...
    expect(
      await normalize([...atLimit, { path: 'extra.txt', bytes: new Uint8Array([1]) }]),
  ...
  it('单文件恰好 1 MiB 通过,超出报 file_too_large', async () => {
    ...
    expect(
      ...
      ok: false,
      error: { code: 'file_too_large', param: 'SKILL.md' },
    });
  });
  ...
  it('总量超出 20 MiB 报 total_too_large', async () => {
    ...
    expect(await normalize(files)).toMatchObject({
      ok: false,
...
describe('normalizeSkillTree 哈希与排序', () => {
  ...
  it('输入顺序打乱不影响 contentSha256(哈希对排序确定)', async () => {
    const files = validFiles().concat([
      { path: 'refs/a.md', bytes: new TextEncoder().encode('a') },
    ]);
  ...
  it('逐文件 sha256 与 totalBytes 统计正确', async () => {
    ...
    if (result.ok) {
      ...
      expect(result.tree.totalBytes).toBe(
        new TextEncoder().encode(SKILL_MD).length +
